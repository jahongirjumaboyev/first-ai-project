...
export default function XonaDrawer({ open, onClose, onSaved }) {
        ...
        <>
            ...
            <div className={`fixed top-0 right-0 h-screen w-full sm:w-90 bg-white dark:bg-[#1e2a3a] z-101 flex flex-col shadow-[-4px_0_24px_rgba(0,0,0,0.18)] transition-transform duration-280 ease-in-out ${visible ? 'translate-x-0' : 'translate-x-full'}`}>
                ...
                <div className="px-6 py-4 border-t border-[#e5e7eb] dark:border-[#2d3748] flex justify-end gap-3 shrink-0">
                    <button onClick={onClose} className="px-6 py-2.5 rounded-[10px] text-sm font-medium border border-[#e5e7eb] dark:border-[#2d3748] bg-transparent text-[#1a1a2e] dark:text-[#e2e8f0] cursor-pointer hover:bg-[#f5f5f5] dark:hover:bg-[#2d3748] transition-colors duration-200">Bekor qilish</button>
                    <button
                        onClick={saveRoom}
                        disabled={saving}
