...
export default function StudentDrawer({ open, onClose, onSaved, editingStudent, apiGroups }) {
    ...
    return (
        <>
            ...
            <div className={`fixed top-0 right-0 bottom-0 w-full sm:w-88 bg-white dark:bg-[#1e2a3a] z-201 flex flex-col shadow-[-6px_0_32px_rgba(0,0,0,0.18)] transition-transform duration-350 ease-in-out ${open ? 'translate-x-0' : 'translate-x-full'}`}>
                ...
                <div className="px-6 pt-5.5 pb-4 border-b border-[#e8e8e8] dark:border-[#2d3748] shrink-0">
                    <div className="flex justify-between items-start">
                        <div>
                            <h2 className="m-0 text-lg font-bold text-[#1a1a2e] dark:text-[#e2e8f0]">{editingStudent ? "Talabani tahrirlash" : "Talaba qo'shish"}</h2>
                            <p className="mt-1 mb-0 text-[13px] text-[#6b7280] dark:text-[#94a3b8]">{editingStudent ? "Talaba ma'lumotlarini yangilang." : "Bu yerda siz yangi talaba qo'shishingiz mumkin."}</p>
                        </div>
                ...
                <div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-4.5">
                    ...
                    <div>
                        ...
                        <input value={form.phone} onChange={e => upd('phone', e.target.value)} type="tel" placeholder="+998901234567" className={inputCls} />
                    </div>
            ...
            {groupModalOpen && (
                <GroupSelectModal
                    groups={apiGroups}
                    initialSelected={form.groups}
                    onClose={() => setGroupModalOpen(false)}
                    onApply={(selected) => { upd('groups', selected); setGroupModalOpen(false) }}
                />
            )}
        </>
    )
